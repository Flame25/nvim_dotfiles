...
function M.cmp_source(name, icon)
	...
	local colors = {
		ok = PotatoVim.ui.fg("Special"),
		error = PotatoVim.ui.fg("DiagnosticError"),
		pending = PotatoVim.ui.fg("DiagnosticWarn"),
	}

	return {
		function()
			return icon or PotatoVim.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
		end,
		cond = function()
			return status() ~= nil
		end,
...
function M.pretty_path(opts)
	...
	return function(self)
		...
		local root = PotatoVim.root.get({ normalize = true })
		local cwd = PotatoVim.root.cwd()
